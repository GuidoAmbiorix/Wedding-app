...
const links = [
  {
    ...
    icon: `<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a4 4 0 00-4-4h-1M9 20H4v-2a4 4 0 014-4h1m4-4a4 4 0 100-8 4 4 0 000 8zm6 0a3 3 0 100-6 3 3 0 000 6z"/></svg>`,
  },
  {
    to: '/admin/confirmaciones',
    label: 'Confirmaciones',
    sub: computed(() => `${stats.value.confirmado} confirmados`),
    icon: `<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>`,
  },
  {
...
const pageTitle = computed(() => {
  const map = {
    ...
    '/admin/confirmaciones': 'Confirmaciones',
...
<template>
    ...
    >
      ...
      <nav class="flex-1 px-2 py-2 space-y-1">
        <RouterLink
          ...
          :key="link.to"
          :to="link.to"
          class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-150 no-underline relative"
      ...
      <!-- Bottom -->
      <div class="px-2 pb-3 border-t border-white/10 pt-3">
        <RouterLink to="/"
    ...
    <div class="flex-1 flex flex-col min-w-0">
      ...
      <header class="h-14 md:h-16 bg-white border-b border-gray-100 flex items-center px-4 md:px-6 gap-3 flex-shrink-0 shadow-sm">
        ...
        <div class="flex-1 min-w-0">
          <h1 class="text-base md:text-lg font-bold text-gray-900 leading-none">{{ pageTitle }}</h1>
          <p class="text-[10px] md:text-xs text-gray-400 mt-0.5 hidden sm:block">Panel de administración</p>
        </div>
        ...
        <div class="hidden lg:flex items-center gap-4">
          ...
          <div class="text-center">
            <p class="text-base font-bold leading-none" style="color:#2d5a27;">{{ stats.confirmado }}</p>
            <p class="text-[10px] text-gray-400 uppercase tracking-wider">Confirmados</p>
          </div>
          <div class="w-px h-8 bg-gray-200" />
          <div class="text-center">
            <p class="text-base font-bold text-amber-500 leading-none">{{ stats.sin_respuesta }}</p>
            <p class="text-[10px] text-gray-400 uppercase tracking-wider">Pendientes</p>
          </div>
        </div>

        <!-- Stats compactas (mobile/tablet) -->
        <div class="flex sm:hidden items-center gap-3 text-xs">
          <span class="font-bold text-emerald-600">{{ stats.confirmado }} ✓</span>
          <span class="text-gray-400">·</span>
          <span class="font-bold text-gray-500">{{ stats.total }}</span>
        </div>
  ...
  <!-- ── BOTTOM NAV (móvil) ── -->
  <nav class="md:hidden fixed bottom-0 inset-x-0 z-30 bg-white border-t border-gray-100 shadow-[0_-1px_12px_rgba(0,0,0,.06)] flex">
    <RouterLink
      v-for="link in links"
      :key="link.to"
      :to="link.to"
    ...
    <!-- Ver sitio -->
    <RouterLink to="/"
      class="flex-1 flex flex-col items-center justify-center py-2.5 gap-1 no-underline text-gray-400 transition-colors"
    >
      <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8">
        <path stroke-linecap="round" stroke-linejoin="round" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/>
      </svg>
      <span class="text-[10px] font-semibold tracking-wide">Sitio</span>
    </RouterLink>
  </nav>
</template>
